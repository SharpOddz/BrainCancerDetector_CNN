...
#Local google colab path and the google drive path
...
#Getting image and resizing it (Additional data augmentations are done later)
...
#Utilizes parallel processing to speed up data preparation
def prepare_data(df, split_name):
    ...
    #Using ThreadPoolExecutor with controlled max_workers to avoid overloading
    with ThreadPoolExecutor(max_workers=8) as executor:
        results = list(executor.map(process_image_path, tasks))
    #Filtering out None values and separate images and labels
...
#Model Architecture
'''
CNN that has 6 convolutional layers, utilizing batch normalizationa and ReLU activations for each layer.
At the end of the sixth convolution, global average pooling is performed and fully connected to the output.
Global average pooling was chosen to reduce parameter count for the fully connected layer.
'''
...
#Displyaing plot of model jistory
if 'history' in globals():
    plot_training_history(history)
else:
    print("History not found. Train the model first")
...
#This will automatically load the best model (checkpoint that was saved) 
if os.path.exists(checkpoint_path):
    print(f"Loading best model from {checkpoint_path}...")
    model = tf.keras.models.load_model(checkpoint_path)
    #Calculates the training and val loss
    if 'history' in globals():
...
#Test evaluation (turn off if tuning hyperparameters)
print("Evaluating on Test Set...")
test_loss, test_acc = model.evaluate(x_test, y_test, verbose=1)
print(f"Test Loss: {test_loss:.4f}")
print(f"Test Accuracy: {test_acc:.4f}")

#Predictions for the train,val, and test sets
